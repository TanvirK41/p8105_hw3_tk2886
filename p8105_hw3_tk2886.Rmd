...
```{r Load the original dataset called accel_data and cleaning the data}
accel_dfa <- 
...
accel_col = 
  new_accel_df %>% ncol()
```
The original dataset that was provided has the *dimension* of **`r original_accel_df`**. We may be more specific and state that the number of *rows* in the original dataset is **`r original_accel_row`**. The number of *cols* in the original data is **`r original_accel_col`**. 
...
**Analysis of** *"Analysis of 24-hour Activity Per Day Plot":* One trend that is indicated from the graph is that the activity count is higher later on night on Wednesday (Wednesday having the highest activity count around the 20th hour of the day), Thursday, Friday, Saturday, and Sunday. At the beginning of the week like Sunday, the activity count is higher in the afternoon (12pm) hour which may indicate that he may go out with his family or friends for brunch or some weekly activity. Another trend that is observed is that around midnight, the activity count  is lower compared to the rest of the hours in the day which makes sense because maybe the individual is resting or not doing as much as work compared to morning and afternoon hours. Another trend that is shown is that around the afternoon hours, the activity count is higher (not the highest) which may indicate that the person may be at his job place working or doing some activity that leads to high activity counts. Another trend that is shown is that around (8hr mark), the activity count on Thursday is high which may indicate that the person wakes up early on that day and he probably goes out for a walk or has weekly task that he needs to complete. 
