...
The dimension of the *instacart dataset* is **1384617, 15**. The
dimensions represents the number of rows and the columns. The number of
*rows* in the instacart dataset are **1384617**. The number of *columns*
in the instacart dataset are **15**. The number of *observations* in the
instacart dataset are **1384617**.
...
a descending order which shows aisles with the most items ordered at the
top of the plot. The top five aisles from the aisles with more than
10,000 items ordered are Fresh Vegetables, Fresh Fruits, Packaged
...
**Table 1:** *Showcasing the Three Most Popular items in Baking
Ingredients Aisle, Dog Food Care Aisle, and Packaged Vegetables Fruits
aisle*
...
Apples, **Pink Lady Apples** are ordered earlier in the day for Sunday,
Monday, Tuesday, Wednesday, Thursday, and Saturday. Friday is the only
day when **Coffee Ice Cream** is ordered earlier in the day.
...
and track health objectives, and develop and evaluate public health
policies and programs. The dataset is not final since the data will be
updated annually as it becomes available.

In the cleaned and tidied version of the brfss\_smart 2010 dataset
called behaviorial\_df, I’m focusing only in the “Overall Health” topic
and including only responses from “Excellent” to “Poor”.

### Analyzing the original brfss\_smart2010 dataet and the filtered and cleaned brfss\_smart dataset.
...
The number of *observations* in the original brfss\_smart2010 dataset
are **134203**. The number of *rows* in the original brfss\_smart2010
dataset are **134203**. The number of *columns* in the original
brfss\_smart2010 dataset are **23.**

For the tidied and filtered brfss dataset, there are **10625**
*observations*.The number of *rows* in the tidied and filtered brfss
dataset are **10625**. The number of *columns* in the tidied and
filtered brfss dataset are **23.**

The key variables between the original brfss\_smart2010 data and the
cleaned and tidied dataset (behaviorial\_df) do not change. We still
have the same number of key variables. We are not omitting any of the
columns. THe *key variable names* are **year, locationabbr,
locationdesc, class, topic, question, response, sample\_size,
data\_value, confidence\_limit\_low, confidence\_limit\_high,
display\_order, data\_value\_unit, data\_value\_type,
data\_value\_footnote\_symbol, data\_value\_footnote, data\_source,
class\_id, topic\_id, location\_id, question\_id, respid,
geo\_location**.
...
**Table 1:** *States in 2002 with 7 or more locations Observed*
...
**Table 2:** *States in 2010 with 7 or more locations Observed*
...
The spaghetti plot is showing the BRFSS data value within the states
across the years from 2002 to 2010. The data values vary within the
states from 2002 to 2010. It is hard to state if there’s a trend
happening for each specific state or across each state if we want to
compare multiple states. The data values vary as we progress from 2002
to 2010. However, we may state that the average BRFSS data value for
some states starts high and then it decreases.
...
The accel\_data dataset represents five weeks of accelerometer data
...
cleaned and tidy version that is created from that specific dataset
...
There is not 50,400 different people or objects but rather the 50,400
represents the 50,400 minutes that was logged in by that one person.
That male individual was observed and measured for 35 days by using an
accelerometer. The information that was provided with the dataset states
that the activity counts consists of each minute of a 24 hour day
starting at midnight which means we have data for the whole day instead
of a specific time interval of the day. We may also divide 50,400 by
1440 (1440 minutes equals to one day) which equals to 35 days which is
the number of days that the male individual was observed for.
...
The original dataset that was provided has the *dimension* of **35,
1443**. We may be more specific and state that the number of *rows* in
the original dataset is **35**. The number of *columns* in the original
data is **1443**.

The tidied dataset that I created has *dimensions* of **50400, 7**. The
...
higher compared to other days. On Saturday the activity is the lowest
and one may infer that the person may not be working that day and it is
his rest day.
...
<img src="p8105_hw3_tk2886_files/figure-gfm/code for creating a single-panel plot-1.png" width="90%" />

*“Analysis of 24-hour Activity Per Day Plot”:* One trend that is
indicated from the graph is that the activity count is higher later at
night on Wednesday (Wednesday having the highest activity count around
the 20th hour of the day), Thursday, Friday, Saturday, and Monday. At
the beginning of the week like Sunday, the activity count is higher in
the afternoon (12pm) hour which may indicate that he may go out with his
family or friends for brunch or some weekly activity. Another trend that
is observed is that around midnight, the activity count is lower
compared to the rest of the hours in the day which makes sense because
maybe the individual is resting or not doing as much as work compared to
morning and afternoon hours. Another trend that is shown is that around
the afternoon hours, the activity count is higher on Monday which may
indicate that the person may be at his job place working or doing some
activity that leads to high activity counts. Another trend that is shown
is that around (8hr mark), the activity count on Thursday is high which
may indicate that the person wakes up early on that day and he probably
goes out for a walk or has weekly task that he needs to complete during
